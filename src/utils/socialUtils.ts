...
export const calculateMoodSimilarity = (
  ...
  // Time proximity (15% weight) - simplified handling here
  const timeSimilarity = 0.15;

  // User activity (10% weight) - simplified handling here
...
export const generateEmotionPreview = (): string => {
  const previews = [
    ...
    'Feeling quite stressed lately, need someone to talk to...',
    'Feeling good, want to share some happy things...',
    'Feeling uncertain about the future, hoping to get support...',
    'Something touching happened today...',
    'Been thinking about the meaning of life lately, feeling confused...',
    'Made a breakthrough at work, feeling excited...',
...
export const generateAIReplySuggestions = (mood: number): string[] => {
  const suggestions: { [key: number]: string[] } = {
    1: [
      'Although it\'s difficult right now, you\'re not alone, we\'re all here to support you',
      'Everyone goes through low periods, believe tomorrow will be better',
      'It\'s already brave of you to share these feelings',
    ],
    2: [
    ...
    4: [
      ...
      'Hope this good mood continues',
    ],
    5: [
      'Your happiness is very contagious!',
      'Awesome! Your good mood makes me happy too',
...
export const processContent = (content: string): string => {
  ...
  processed = processed.replace(/\d{11}|\d{3}-\d{4}-\d{4}/g, '[Contact Info]');
  processed = processed.replace(/\S+公司|\S+学校/g, '[Work/Study Place]');
